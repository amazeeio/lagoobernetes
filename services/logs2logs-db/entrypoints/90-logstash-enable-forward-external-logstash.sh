...
if [ "$LOGS2LOGSDB_LAGOOBERNETESLOGS_EXTERNAL_LOGSTASH_ENABLED" == "true" ]; then
  if [ -z $LOGS2LOGSDB_LAGOOBERNETESLOGS_EXTERNAL_LOGSTASH_HOST ]; then echo "LOGS2LOGSDB_LAGOOBERNETESLOGS_EXTERNAL_LOGSTASH_HOST is not set, but needed for LOGS2LOGSDB_LAGOOBERNETESLOGS_EXTERNAL_LOGSTASH_ENABLED"; exit 1; fi
  if [ -z $LOGS2LOGSDB_LAGOOBERNETESLOGS_EXTERNAL_LOGSTASH_PORT ]; then echo "LOGS2LOGSDB_LAGOOBERNETESLOGS_EXTERNAL_LOGSTASH_PORT is not set, but needed for LOGS2LOGSDB_LAGOOBERNETESLOGS_EXTERNAL_LOGSTASH_ENABLED"; exit 1; fi

  sed -i -r 's/#\?LOGS2LOGSDB_LAGOOBERNETESLOGS_EXTERNAL_LOGSTASH_ENABLED(.*)/\1/' pipeline/lagoobernetes-logs.conf
fi
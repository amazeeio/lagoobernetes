...
if [[ $(kubectl -n ${NAMESPACE} get deploy --no-headers=true -o name -l lagoobernetes/service=${SERVICE_NAME}| wc -l) -gt 0 ]]; then
  DEPLOYMENTCONFIG=$(kubectl -n ${NAMESPACE} get deploy -l lagoobernetes/service=${SERVICE_NAME} -o name)
...
POD=$(kubectl -n ${NAMESPACE} get pods -l lagoobernetes/service=${SERVICE_NAME} -o json | jq -r '[.items[] | select(.metadata.deletionTimestamp == null) | select(.status.phase == "Running")] | first | .metadata.name // empty')

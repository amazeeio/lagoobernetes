...
!!!hint
    **Note**: this document will talk about Drupal and Laravel base images as examples, as it was originally written for a client who uses those technologies in their Lagoobernetes projects. It will be expanded to cover the contents of other base images, but none of the processes differ, no matter what the content of your base image.
...
Other pieces of your project are defined in [`docker-compose.yml`](https://lagoobernetes.readthedocs.io/en/latest/using_lagoobernetes/docker-compose_yml/). For example, if you have a Drupal project, you need the Drupal image, but you also need MariaDB, Solr, Redis, and Varnish. We have versions of these services optimized for Drupal, all of which are included in `docker-compose.yml`.
...
3. **Images Test** - This will run the `make images_test` target, which will differ depending on the images being tested. In most cases this is a very simple test to ensure that the images can be started and interacted with in some way \(installing Drupal, listing files, etc.\)
...
For any modules now added to the base image, we need to ensure that they’re enabled on the template-based derived images. This is done by adding the module to the Lagoobernetes Bundle module located at `./web/modules/lagoobernetes/lagoobernetes_bundle`. Specifically, it requires you to add it as a dependency to the `dependencies` section of the `lagoobernetes_bundle.info.yml` file. The Lagoobernetes Bundle module is a utility module that exists only to help enforce dependencies across derived images.

Here we open `web/modules/contrib/lagoobernetes/lagoobernetes_bundle/lagoobernetes_bundle.info.yml` and add `clamav:clamav` as a dependency:

![Adding ClamAV as a dependency of Lagoobernetes Bundle.](/images/3.png)

Adding a dependency to this will ensure that whenever the Lagoobernetes Bundle module is enabled on the derived image, its dependencies \(in this case, the just-added clamAV module\) will also be enabled. This is enforced by a post-rollout script which enables `lagoobernetes_bundle` on the derived images when they are rolled out.

#### Step 2.3 - Test

This will depend on what you’re testing. In the case of adding the ClamAV module, we want to ensure that in the base image, the module is downloaded, and that the Lagoobernetes Bundle module enables clamAV when it is enabled.
...
And then we check that when we enable the `lagoobernetes_bundle` module, it enables `clamav` by running:

```bash
drush pm-enable lagoobernetes_bundle -y
```

![Running \`drush pm-enable lagoobernetes\_bundle -y\` and seeing that it also enables ClamAV](/images/5.gif)

!!!warning
    **Note:** You’ll see that there is a JWT error in the container above. You can safely ignore this in the demonstration above - but, for background, you will see this error when there is no Lagoobernetes environment for the site you’re working on.
...
1. Visit your Lagoobernetes Jenkins instance.

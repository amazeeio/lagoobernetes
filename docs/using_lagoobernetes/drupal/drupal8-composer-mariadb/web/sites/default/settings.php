...
if (getenv('LAGOOBERNETES') && (file_exists($app_root . '/modules/contrib/search_api_solr') || file_exists($app_root . 'modules/search_api_solr'))) {
...
// Lagoobernetes Redis connection if Drupal Redis module exists and PHP Redis Module is loaded.
if (getenv('LAGOOBERNETES') && (file_exists($app_root . '/modules/contrib/redis') || file_exists($app_root . 'modules/redis')) && extension_loaded('redis')) {
...
if (getenv('LAGOOBERNETES_ROUTES')) {
  ...
  // Escape dots, remove schema, use commas as regex separator.
    '^' . str_replace(['.', 'https://', 'http://', ','], ['\.', '', '', '|'], getenv('LAGOOBERNETES_ROUTES')) . '$',
  );
}

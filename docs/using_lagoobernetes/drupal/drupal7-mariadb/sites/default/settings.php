...
/**
 * @file
 * Lagoobernetes Drupal 7 configuration file.
 *
 ...
 */
 ...
 if (getenv('LAGOOBERNETES')) {
  ...
  if (getenv('LAGOOBERNETES')) {
    ...
    $conf['search_api_override_servers']['solr']['name'] = 'Lagoobernetes Solr - Environment:' . getenv('LAGOOBERNETES_PROJECT');
  ...
  if (getenv('LAGOOBERNETES')) {
    ...
    $conf['varnish_control_key'] = getenv('VARNISH_SECRET') ?: 'lagoobernetes_default_secret';
  ...
  // Hash Salt.
  if (getenv('LAGOOBERNETES')) {
    $drupal_hash_salt = hash('sha256', getenv('LAGOOBERNETES_PROJECT'));
  }
